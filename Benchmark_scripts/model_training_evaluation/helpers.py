...
def PlotROCCurve(probs,y_test_roc, ci= 95, random_seed=0):
    # Input validation: ensure arrays are 1-D and same length
    probs = np.asarray(probs)
    y_test_roc = np.asarray(y_test_roc)
    if probs.ndim > 1 and probs.shape[1] == 2:
        # if user passed full predict_proba output, take positive class
        probs = probs[:, 1]

    if probs.ndim != 1 or y_test_roc.ndim != 1:
        print("PlotROCCurve: expected 1-D arrays for probs and y_test_roc")
        return None

    if len(probs) != len(y_test_roc):
        print(f"PlotROCCurve: length mismatch: probs={len(probs)} vs y_test_roc={len(y_test_roc)}")
        return None

    # If only one class present in y, ROC can't be computed
    if np.unique(y_test_roc).size < 2:
        print("PlotROCCurve: y_test_roc contains only one class; ROC is undefined.")
        return None
    ...
    try:
        ...
        # Ensure a fresh figure is created (helps in notebooks)
        plt.figure()
        ...
        # Catch any plotting/metric errors and print diagnostics
        print(f"PlotROCCurve: encountered an error: {e}")
        import traceback
        traceback.print_exc()
        return None
