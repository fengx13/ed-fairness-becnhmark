"""Build the Stanford / MC-MED master encounter-level dataset.

Reads the raw MC-MED CSV exports (visits, meds, past medical history, labs),
harmonizes them to the FairED schema, engineers history/comorbidity/lab
features, computes rule-based clinical scores, labels utilization and
disease-specific outcomes, and writes ``master_dataset.csv`` to the configured
output directory.

Run as::

    python -m fair_ed.data.build_stanford --config configs/paths.yaml
"""
import argparse
import os
...
from fair_ed.data.disease_defs import DISEASE_DEFS
from fair_ed.data.stanford import (
    adjust_time_by_subtracting_year,
    apply_outcome_detection,
    build_comorbidity_code_dictionary,
    calculate_comorbidities_batch,
    calculate_medication_history,
    calculate_visit_history,
    convert_9to10,
    convert_10to9,
    encode_chief_complaints,
    merge_lab_data_with_visits,
    process_lab_data,
    split_clean_icd,
)
from fair_ed.scores.clinical_scores import (
    add_score_CART,
    add_score_CCI,
    add_score_MEWS,
    add_score_NEWS,
...
def build(paths: dict) -> "pd.DataFrame":
    """Build the master dataset and write it to ``paths['output_dir']``."""
    df_visits = pd.read_csv(paths['visits_path'])
    df_meds = pd.read_csv(paths['meds_path'])
    df_pmh = pd.read_csv(paths['pmh_path'])
    df_labs = pd.read_csv(paths['labs_path'])
    ...
    df_master.to_csv(os.path.join(paths['output_dir'], 'master_dataset.csv'), index=False)
    return df_master
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        default=None,
        help="Path to paths.yaml (defaults to configs/paths.yaml or $FAIRED_PATHS).",
    )
    args = parser.parse_args()
    build(load_paths(args.config))
